...
Item {
    ...
    Loader {
        id: pluginLoader
        anchors.fill: parent
        function reload() { source = ""; source = "../qml/RuntimeStatistics.qml"; }
    }
    ...
    TestCase {
        ...
        function injectStats(entries) {
            ...
            sub.injectMessage({ statistics: entries });
        }
        ...
        function init() {
            ...
            Ros2.registerService("/mock_cm/list_controllers",
                ...
                function(req) {
                    return Ros2.createEmptyServiceResponse(
                        "controller_manager_msgs/srv/ListControllers");
                });
        ...
        function test_plugin_loads() {
            verify(plugin !== null, "RuntimeStatistics plugin should load");
        }

        function test_controller_manager_discovery() {
            Ros2.registerService("/another_cm/list_controllers",
                "controller_manager_msgs/srv/ListControllers",
                function(req) {
                    return Ros2.createEmptyServiceResponse(
                        "controller_manager_msgs/srv/ListControllers");
        ...
        function test_message_processing() {
            ...
            injectStats([
                { name: "controller_a.stats/execution_time/current_value", value: 50.0 },
                { name: "controller_b.stats/execution_time/current_value", value: 100.0 },
                { name: "some_other_stat/not_execution_time", value: 999.0 }
            ]);

            var listView = find("rsStatsListView");
            verify(listView !== null, "ListView should exist");
            tryCompare(listView.model, "count", 2, 2000,
                       "Only execution_time entries should appear (2 of 3)");
        }
        ...
        function test_label_parsing() {
            ...
            injectStats([
                { name: "joint_state_broadcaster.stats/execution_time/current_value", value: 10 },
                { name: "arm_interface.stats/read_cycle/execution_time/current_value", value: 20 },
                { name: "arm_interface.stats/write_cycle/execution_time/current_value", value: 30 }
            ]);
        ...
        function test_pause_resume() {
            ...
            tryCompare(listView.model, "count", 2, 2000,
                       "New element should appear after resume");
        }
        ...
        function test_clear_and_reset() {
            ...
            tryCompare(listView.model, "count", 0, 2000,
                       "Model should be empty after clear");

            var samplesLabel = find("rsSamplesLabel");
            tryCompare(samplesLabel, "text", "Samples: 0", 1000,
                       "Samples counter should reset");
        }
        ...
        function test_clear_when_paused() {
            ...
            injectStats([
                { name: "ctrl_a.stats/execution_time/current_value", value: 15 },
                { name: "ctrl_b.stats/execution_time/current_value", value: 25 }
            ]);
            ...
            mouseClick(clearBtn);
            tryCompare(listView.model, "count", 0, 2000,
                       "Model should be empty after clear while paused");

            // Wait longer than update timer + animation duration and verify stays empty
            wait(1500);
            compare(listView.model.count, 0,
                    "Model should remain empty with no new messages arriving");

            // Also verify the elements label
            var elementsLabel = find("rsElementsLabel");
            compare(elementsLabel.text, "0 elements",
                    "Elements counter should show zero");
        }
        ...
        function test_clear_stale_entries() {
            ...
            injectStats([
                { name: "a.stats/execution_time/current_value", value: 10 },
                { name: "b.stats/execution_time/current_value", value: 20 },
                { name: "c.stats/execution_time/current_value", value: 30 }
            ]);
            ...
            tryCompare(listView.model, "count", 1, 2000,
                       "Exactly one element after clear and partial repopulation");

            compare(listView.model.get(0).label, "only_one",
                    "The single element should be the newly published one");
        }
        ...
        function test_ring_buffer_caps_samples() {
            ...
            tryVerify(function() { return find("rsStatisticsSubscription") !== null; }, 2000);
        ...
        function test_set_max_samples_trims() {
            ...
            compare(listView.model.get(0).min, 300,
                    "Oldest samples should have been trimmed by setMaxSamples");
        }
        ...
        function test_namespace_change_clears() {
            ...
            Ros2.registerService("/other_cm/list_controllers",
                "controller_manager_msgs/srv/ListControllers",
                function(req) {
                    return Ros2.createEmptyServiceResponse(
                        "controller_manager_msgs/srv/ListControllers");
            ...
            for (var i = 0; i < combo.model.length; i++) {
                if (combo.model[i] === "/other_cm") { idx = i; break; }
            }
            ...
            tryCompare(listView.model, "count", 0, 2000,
                       "Switching namespace should clear old data");

            var samplesLabel = find("rsSamplesLabel");
            compare(samplesLabel.text, "Samples: 0",
                    "Samples counter should reset on namespace change");
        }
        ...
        function test_nonfinite_values_filtered() {
            ...
            injectStats([
                { name: "ok.stats/execution_time/current_value", value: 42 },
                { name: "nan.stats/execution_time/current_value", value: NaN },
                { name: "inf.stats/execution_time/current_value", value: Infinity },
                { name: "neginf.stats/execution_time/current_value", value: -Infinity }
            ]);

            var listView = find("rsStatsListView");
            tryCompare(listView.model, "count", 1, 2000,
                       "Only the finite-valued entry should appear");
            compare(listView.model.get(0).label, "ok",
                    "Only the valid entry should be in the model");
        }
        ...
        function test_empty_state_message() {
            ...
            injectStats([
                { name: "unrelated_stat/something_else", value: 99 }
            ]);
            ...
            var samplesLabel = find("rsSamplesLabel");
            compare(samplesLabel.text, "Samples: 0",
                    "totalSamples should be 0 since nothing matched");
        }
