...
Item {
    ...
    QtObject {
        id: contextObj
        property string action_server: ""
        property string move_group: ""
        property real velocity_scale: 0.1
        property real acceleration_scale: 0.1
        property real planning_time: 5.0
        property int planning_attempts: 5
    }
    ...
    Loader {
        id: pluginLoader
        anchors.fill: parent
        function reload() { source = ""; source = "../qml/MoveItController.qml"; }
    }
    ...
    TestCase {
        ...
        readonly property string mockUrdf:
            '<?xml version="1.0"?>' +
        ...
        readonly property string mockSrdf:
            '<?xml version="1.0"?>' +
            '<robot name="test_robot">' +
            '  <group name="arm">' +
        ...
        function setupFullState() {
            ...
            contextObj.action_server = "/move_group";
            contextObj.move_group = "arm";
            pluginLoader.reload();
        ...
        function init() {
            ...
            Ros2.registerAction("/move_group", "moveit_msgs/action/MoveGroup",
            ...
            // Reset context
            contextObj.action_server = "";
            contextObj.move_group = "";
            contextObj.velocity_scale = 0.1;
            contextObj.acceleration_scale = 0.1;
            contextObj.planning_time = 5.0;
            contextObj.planning_attempts = 5;
        ...
        function test_action_server_refresh() {
            ...
            Ros2.registerAction("/second_move_group", "moveit_msgs/action/MoveGroup",
                function(goal, callbacks) {});
        ...
        function test_chain_joint_resolution() {
            ...
            for (var i = 0; i < jointList.count; i++) {
                verify(jointList.model.get(i).name !== "fixed_joint",
                    "Fixed joint should not be in the list");
            }
        }

        function test_subgroup_joint_resolution() {
            contextObj.action_server = "/move_group";
            contextObj.move_group = "combined";
            pluginLoader.reload();
            tryVerify(function() { return pluginLoader.status === Loader.Ready; }, 2000);
            waitForDiscovery();
        ...
        function test_apply_named_pose() {
            ...
            var posesFlow = find("moveitNamedPosesFlow");
            tryVerify(function() { return posesFlow.visible; }, 3000);
            ...
            tryVerify(function() {
                ...
                return Math.abs(j1.goal - 1.0) < 0.02
                    && Math.abs(j2.goal - (-0.5)) < 0.02
                    && Math.abs(j3.goal - 0.5) < 0.02;
        ...
        function test_joint_state_subscription() {
            ...
            var jointList = find("moveitJointListView");
            tryVerify(function() { return jointList.count === 3; }, 3000);
        ...
        function test_continuous_joint_wrapping() {
            ...
            tryVerify(function() {
                ...
                return Math.abs(j1.position - 4.0) < 0.02
                    || Math.abs(j1.position) < 0.02; // rounded to 0 if || 0.0 kicks in
        ...
        function test_srdf_before_urdf_order() {
            ...
            var groupCombo = find("moveitMoveGroupComboBox");
            tryVerify(function() { return groupCombo.count === 3; }, 3000,
                "Move groups should be available from SRDF alone");
        ...
        function test_namespace_aware_topic_discovery() {
            ...
            Ros2.registerAction("/athena/move_group", "moveit_msgs/action/MoveGroup",
                function(goal, callbacks) { lastCallbacks = callbacks; });
            Ros2.registerTopic("/joint_states", "sensor_msgs/msg/JointState");
            ...
            contextObj.action_server = "/athena/move_group";
            contextObj.move_group = "arm";
            pluginLoader.reload();
            tryVerify(function() { return pluginLoader.status === Loader.Ready; }, 2000);
            waitForDiscovery();
            ...
            var urdfLabel = find("moveitUrdfStatusLabel");
            tryCompare(urdfLabel, "text", "URDF: Loaded", 3000,
                "Plugin should subscribe to namespaced URDF topic");
            ...
            var srdfLabel = find("moveitSrdfStatusLabel");
            tryCompare(srdfLabel, "text", "SRDF: Loaded", 3000,
                "Plugin should subscribe to namespaced SRDF topic");

            // Verify joints resolved
            var jointList = find("moveitJointListView");
            tryVerify(function() { return jointList.count === 3; }, 5000,
                "Joints should populate via namespaced topics");
        }
        ...
        function test_action_server_change_clears_state() {
            ...
            tryVerify(function() { return jl.model.get(0).initialized; }, 3000);
            ...
            Ros2.registerAction("/other_move_group", "moveit_msgs/action/MoveGroup",
                function(goal, callbacks) {});

            contextObj.action_server = "/other_move_group";
            pluginLoader.reload();
            tryVerify(function() { return pluginLoader.status === Loader.Ready; }, 2000);
            waitForDiscovery();
            ...
            tryCompare(urdfLabelAfter, "text", "URDF: Waiting...", 3000,
                "URDF should reset when action server changes");

            var srdfLabelAfter = find("moveitSrdfStatusLabel");
            tryCompare(srdfLabelAfter, "text", "SRDF: Waiting...", 3000,
                "SRDF should reset when action server changes");
        ...
        function test_execute_sends_goal() {
            ...
            tryVerify(function() { return executeBtn.enabled; }, 5000,
                "Execute button should be enabled when action is ready");

            mouseClick(executeBtn);

            // Verify the action handler received the goal
            tryVerify(function() { return lastCallbacks !== null; }, 3000,
                "Action handler should receive goal");
        ...
        function test_goal_acceptance() {
            ...
            tryCompare(executeBtn, "text", "Cancel", 3000,
                "Button should show 'Cancel' after goal accepted");
        }
        ...
        function test_goal_result_success() {
            ...
            tryCompare(executeBtn, "text", "Execute", 3000,
                "Button should return to 'Execute' after success");
        ...
        function test_goal_result_planning_failed() {
            ...
            var errorBanner = find("moveitErrorBannerRect");
            tryVerify(function() { return errorBanner.visible; }, 3000,
                "Error banner should be visible on planning failure");
        ...
        function test_goal_result_control_failed() {
            ...
            lastCallbacks.onResult({
                status: 4,
                result: { error_code: { val: -4 } }
            });

            var errorBanner = find("moveitErrorBannerRect");
            tryVerify(function() { return errorBanner.visible; }, 3000,
                "Error banner should be visible on control failure");

            var errorTitle = find("moveitErrorTitle");
            tryCompare(errorTitle, "text", "Controller Error", 3000,
                "Control failure should show 'Controller Error' title");
        ...
        function test_goal_rejected() {
            ...
            // Button should return to "Execute"
            tryCompare(executeBtn, "text", "Execute", 3000,
                "Button should return to 'Execute' after rejection");

            // Error banner should show rejection message
            var errorBanner = find("moveitErrorBannerRect");
            tryVerify(function() { return errorBanner.visible; }, 3000,
                "Error banner should show on goal rejection");
        ...
        function test_goal_cancellation() {
            ...
            // Click cancel
            mouseClick(executeBtn);
            tryVerify(function() { return Ros2._lastActionCancelled === true; }, 3000,
        ...
        function test_context_persistence() {
            ...
            contextObj.action_server = "/move_group";
            contextObj.move_group = "arm";
            contextObj.velocity_scale = 0.5;
            contextObj.acceleration_scale = 0.3;
            contextObj.planning_time = 10.0;
            contextObj.planning_attempts = 8;
        ...
        function test_status_bar_labels() {
            ...
            tryCompare(actionLabelAfter, "text", "Action: Ready", 3000,
                "Action should be ready after setting action server");
        }
        ...
        function test_error_banner_dismissed_on_new_goal() {
            ...
            var errorBanner = find("moveitErrorBannerRect");
            tryVerify(function() { return errorBanner.visible; }, 3000,
                "Error banner should be visible after failure");
            ...
            tryVerify(function() { return !errorBanner.visible; }, 3000,
                "Error banner should be dismissed when new goal is accepted");
        }
    }
}
